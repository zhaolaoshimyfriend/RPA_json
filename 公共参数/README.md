...
**用途**：作为多数任务的公共入参，用于向电子税务局执行登录或标识当前操作主体。平台在创建任务时，可将 `loginInfo` 作为 `params` 的一部分传入；RPA 据此完成登录或切换企业/身份。
...
## declarationContext.json（申报上下文）

**用途**：描述电子税务局申报时所需的**企业信息**和**税款所属期信息**。各类申报任务（如企业所得税预缴、印花税、增值税等）均需携带此对象，以标识「向谁申报、申报哪个税款所属期」。

**报文路径**：`公共参数/declarationContext.json`
...
**用途**：描述 RPA 执行时要运行的工作流。平台在创建任务时，通过此对象指定 RPA 应执行的流程、步骤或脚本等。
